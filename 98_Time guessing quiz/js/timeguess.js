...
stop.addEventListener("click",
  function() {
   ...
   if (timer.textContent.substring(0,5)==="00:10"){
     ...
     //backgraund///
     const firewoeks = document.getElementsByTagName("body");
     firewoeks[0].style.backgroundImage = "url('img/fireworks.gif')";
     firewoeks[0].style.backgroundColor = "rgba(0, 0, 0 ,0)";
   }
   else {
    soundControl("start", "sound/stop1.mp3");
   }
...
reset.addEventListener("click",
  function() {
    ...
    stopTime = 0;
    const firewoeks = document.getElementsByTagName("body");
     firewoeks[0].style.backgroundImage = "";
     firewoeks[0].style.backgroundColor = "rgba(233, 168, 227, 0.6)";
  }
);
...
let w_sound
let music
function soundControl(stutus, w_sound) {
  if(stutus === "start") {
